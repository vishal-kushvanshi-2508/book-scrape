...
def extract_data_from_html(html_content):
    ...
    book_data = tree.xpath("//ol[@class='row']//article[@class='product_pod']")

    base_url = "https://books.toscrape.com/"
    for data in book_data:
        dict_data= {}

        dict_data["book_name"] = data.xpath(".//h3//a/@title")[0].strip()

        dict_data["price"] = data.xpath(".//p[@class='price_color']/text()")[0].strip()

        dict_data["website_link"] = base_url + "catalogue/" +  data.xpath(".//a/@href")[0].strip().replace("catalogue/", "")

        dict_data["image_url"] = base_url +  data.xpath(".//a//img/@src")[0].strip()
        
        dict_data["available"] = data.xpath(".//p[@class='instock availability']/text()")[1].replace("\n", "").strip()
...
def create_html_files(book_data_list):
    ...
    folder_path = r"D:/vishal_kushvanshi/book_scrape/"
